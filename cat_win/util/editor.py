...
class Editor:
    ...
    def _open(curse_window, file: str, file_encoding: str, write_func) -> bool:
        """
        See Editor.open() method
        """
        ...
        try:
            curses_CTL_UP = curses.CTL_UP
            curses_CTL_DOWN = curses.CTL_DOWN
        ...
        try:
            line_sep = Editor.get_newline(file)
        ...
        while True:
            ...
            try:
                if error_bar:
                    curse_window.addstr(max_y - 2, 0, error_bar, curses.color_pair(2))
                    curse_window.addstr(max_y - 2, len(error_bar), " " * (max_x - len(error_bar) - 1), curses.color_pair(2))
                ...
                curse_window.addstr(max_y - 1, 0, status_bar, curses.color_pair(1))
                curse_window.addstr(max_y - 1, len(status_bar), " " * (max_x - len(status_bar) - 1), curses.color_pair(1))
            except curses.error:
                pass
            ...
            elif char in [ord('\b'), curses.KEY_BACKSPACE]:
                unsaved_progress = True
            ...
            # delete (rest of) line (Ctrl-DEL)
            elif char in [520, 527]:
                unsaved_progress = True
                window_content[cur_row] = window_content[cur_row][:cur_col]
                ...
                if cur_col < len(window_content[cur_row]):
                    ...
                    window_content[cur_row] += window_content[cur_row+1]
                    del window_content[cur_row+1]
                ...
                cur_row += 1
            elif char == curses_CTL_UP and cur_row >= 10:
                cur_row -= 10
            elif char == curses_CTL_DOWN and cur_row < len(window_content)-10:
                cur_row += 10
    ...
    def open(file: str, file_encoding: str, write_func, on_windows_os: bool) -> bool:
        """
