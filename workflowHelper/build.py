...
command = f'pyinstaller ./cat_win/__main__.py --onefile --clean --dist ./bin_temp/{platform_name} --icon ./bin/cat_icon.ico --version-file ./bin/catwversionfile -n catw'.split(' ')
...
command = f'pyinstaller ./cat_win/shell.py --onefile --clean --dist ./bin_temp/{platform_name} --icon ./bin/cat_icon.ico --version-file ./bin/catsversionfile -n cats'.split(' ')
# try pyinstaller 3 times at most...
for _ in range(3):
    try:
        sub = subprocess.run(command, check=True)
