...
command = f'pyinstaller ./cat_win/__main__.py --onefile --clean --dist ./temp/{platform_name} --icon ./img/cat_icon.ico --version-file ./temp/catwversionfile -n catw'.split(' ')
...
command = f'pyinstaller ./cat_win/shell.py --onefile --clean --dist ./temp/{platform_name} --icon ./img/cat_icon.ico --version-file ./temp/catsversionfile -n cats'.split(' ')
# try pyinstaller 3 times at most...
for _ in range(3):
    try:
        sub = subprocess.run(command, check=True)
